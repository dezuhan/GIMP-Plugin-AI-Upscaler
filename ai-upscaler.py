...
def _find_bash():
    ...
    if _BASH_EXE is not None:
        return _BASH_EXE
    env = _load_env()
    candidate = env.get("GIT_BASH_PATH", "")
...
def _to_bash_path(path):
    ...
    if drive:
        return "/" + drive[0].lower() + "/" + rest.replace("\\", "/").lstrip("/")
    return path.replace("\\", "/")
...
def _build_command(script, args):
    ...
    if IS_WINDOWS:
        return [_find_bash(), _to_bash_path(script)] + args
    return [script] + args
...
def _run_worker_with_progress(title, args):
    ...
    def _reader():
        for line in iter(proc.stderr.readline, ""):
            stripped = re.sub(r"\x1b\[[0-9;]*[a-zA-Z]|\r", "", line).strip()
            if stripped:
                stderr_lines.append(stripped)
                # Show first meaningful info line as progress text
                if "Downloading" in stripped or "Using" in stripped or "Error" in stripped:
                    last_msg[0] = stripped
    ...
    while proc.poll() is None:
        if last_msg[0]:
            Gimp.progress_set_text("%s", last_msg[0][:80])
        Gimp.progress_pulse()
        time.sleep(0.12)
...
class AIUpscalerGPU(Gimp.PlugIn):
    ...
    def do_create_procedure(self, name):
        ...
        procedure.set_menu_label(_("Upscaler..."))
